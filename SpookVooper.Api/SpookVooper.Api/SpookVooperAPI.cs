...
namespace SpookVooper.Api
{
    ...
    public static class SpookVooperAPI
    {
        ...
        public static class Economy
        {
            ...
            public static async Task<List<OwnerData>> GetOwnerData(string ticker)
            {
                string response = await GetData($"https://api.spookvooper.com/Eco/GetOwnerData?ticker={ticker}");

                List<OwnerData> results = null;

                try
                {
                    results = JsonConvert.DeserializeObject<List<OwnerData>>(response);
                }
#pragma warning disable 0168
                catch (System.Exception e)
                {
